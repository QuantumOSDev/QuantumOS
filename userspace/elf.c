...
{
        quantum_info(__FILE__, 1, " ELF    ", "Unsupported ELF File Class");
		return 0;
	}
	else if (header->e_ident[EI_DATA] != ELFDATA2LSB)
{
        quantum_info(__FILE__, 1, " ELF    ", "Unsupported ELF File byte order");
		return 0;
	}
	else if (header->e_machine != EM_386)
{
        quantum_info(__FILE__, 1, " ELF    ", "Unsupported ELF File target");
		return 0;
	}
	else if (header->e_ident[EI_VERSION] != EV_CURRENT)
{
        quantum_info(__FILE__, 1, " ELF    ", "Unsupported ELF File version");
		return 0;
	}
	else if (header->e_type != ET_REL && header->e_type != ET_EXEC)
{
        quantum_info(__FILE__, 1, " ELF    ", "Unsupported ELF File type");
		return 0;
	}
}
...
int load_elf(char* data, elf32_ehdr_t* header)
{
    ...
    {
        quantum_info(__FILE__, 1, " ELF    ", "This is not an ELF File");
        return INVALID_ELF;
    }

    if (!is_elf_correct(header))
    {
        quantum_info(__FILE__, 1, " ELF    ", "This is not an correct ELF File");
        return NCORRECT_ELF;
    }

    quantum_info(__FILE__, 0, " ELF    ", "Fully correct file");
